...
def test_one_user(x):
    ...
    if not is_val:
        top_20_items = heapq.nlargest(20, test_items, key=lambda i: rating[i])

        return get_performance(user_pos_test, r, auc, Ks), u, top_20_items
    else:
...
def test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if not is_val:
            for re, u, top_items in batch_result:
    ...
    if is_val:
        return result
    return result, user_recommendations
